  ...
  try {
    ...
    if (alreadydonerating) {
      return res.status(403).json({
        success: false,
        message:
          " you are already submit your rating and review for this course",
      });
    }
    ...
    return res.status(200).json({
      success: true,
      message: "rating and review creater successfully",
      ratingAndreviewfield,
    });
  } catch (error) {
...
exports.allRating = async (req, res) => {
  try {
    ...
    const allrating = await ratingAndReview
      .find({})
      .sort({ rating: "desc" })
      .populate({
        path: "user",
    ...
    return res.status(500).json({
      success: false,
      error: error.message,
      message: "error in all reating and review fatching",
    });
  }
};
